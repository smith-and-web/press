...
Font files and OFL notices are local. Preserve the full fallback stacks in the CSS. `design-system/fonts.css` serves the canonical variable TTFs, every axis and weight, for an application that bundles its assets. `design-system/fonts-web.css` serves the same five faces as WOFF2 from `assets/fonts/web/` for delivery over a network, under the **website weight contract**: Fraunces 500–600, Newsreader and Inter 400–700 in roman and italic, optical size kept whole, and Fraunces's SOFT and WONK pinned at their defaults. Each web face is split by `unicode-range` into a Latin file and a Rest file, so a Latin page downloads Latin and no glyph is dropped. A web surface that needs a weight outside the contract changes it in `npm run fonts:build` first; the browser would otherwise clamp or synthesize it. A consumer must not declare these family names against fonts vendored from anywhere else. The foundation hero scales from 32–60px, H1 from 32–44px, H2 from 24–32px, and H3 is 20px. The application has its own heading ceiling of 48px. Small metadata sizes are inherited exceptions, not permission to shrink body text.

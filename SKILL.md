...
Resolve paths from the Press package root. If this skill is pasted into an integration without its files, locate the selected Press design-system folder or linked repository first. Never invent an installation path or assume these references were loaded automatically. Start with `DESIGN.md`, then read only the component API or example needed by the task.
